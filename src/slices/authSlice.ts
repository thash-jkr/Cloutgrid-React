...
function persistSession(refresh: string, user: UserContainer, type: string) {
  localStorage.setItem("refresh", refresh);
  localStorage.setItem("user", JSON.stringify(user));
  localStorage.setItem("type", type);
}
function persistUser(user: UserContainer) {
  localStorage.setItem("user", JSON.stringify(user));
}
...
export const initializeAuth = createAsyncThunk<AuthState>(
  "auth/initializeAuth",
  async () => {
    ...
    try {
      const response = await apiClient.post<{
        access: string;
        refresh?: string;
      }>("/token/refresh/", { refresh }, { requireAuth: false });
...
>("auth/login", async ({ email, password, type }, { rejectWithValue }) => {
  try {
    ...
    return { response: { ...response.data, user: mapUserContainer(response.data.user) }, type };
  } catch (error) {
...
>(
  "auth/updateProfile",
  async ({ type, data, imageBlob }, { rejectWithValue }) => {
    try {
      ...
      const response = await apiClient.put<UserContainerDTO>(
        `/profile/${type}/`,
        formData,
      );
      return mapUserContainer(response.data);
    } catch (error) {
      return rejectWithValue((error as Error).message);
    }
  },
...
>("auth/register", async ({ type, data }, { rejectWithValue }) => {
  try {
    await apiClient.post(`/register/${type}/`, buildFormData(data), {
      requireAuth: false,
    });
  } catch (error) {
    return rejectWithValue((error as Error).message);
...
>("auth/handleOTP", async ({ type, data }, { rejectWithValue }) => {
  try {
    await apiClient.post(`/otp/${type}/`, buildFormData(data), {
      requireAuth: false,
    });
  } catch (error) {
    return rejectWithValue((error as Error).message);
...
export const deleteAccount = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("auth/deleteAccount", async (type, { rejectWithValue }) => {
  try {
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    saveUser(state, action: PayloadAction<UserContainer>) {
